...
GLYellowSphere::GLYellowSphere(float center_x, float center_y, float center_z, float radius, int rows, int segments)
{
...
*/
void GLYellowSphere::initShader(void)
{
	// This loads the shader program from a file
	_program = LoadAndCreateShaderProgram("./spotlight_yellow.vs", "./spotlight_yellow.fs");
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	// Material
	_material._diffuse_material = glm::vec3(0.3, 0.3, 0.1);
	_material._ambient_material = glm::vec3(0.3, 0.3, 0.1);
	...
	_light_source1._lightPos = glm::vec4(30.0, -20.0, 50.0, 1.0);
	_light_source1._ambient_intensity = 0.0;
	_light_source1._specular_intensity = 0.3;
	...
	_light_source1._ambientIdx = glGetUniformLocation(_program, "ambient_intensity_1");
	_light_source1._diffuseIdx = glGetUniformLocation(_program, "diffuse_intensity_1");
	_light_source1._specularIdx = glGetUniformLocation(_program, "specular_intensity_1");
	_light_source1._attenuation_coeffIdx = glGetUniformLocation(_program, "attenuationCoefficient_1");
	_light_source1._lightPosIdx = glGetUniformLocation(_program, "light_position_1");
	_light_source1._cone_angleIdx = glGetUniformLocation(_program, "cone_angle_1");
	_light_source1._cone_directionIdx = glGetUniformLocation(_program, "cone_direction_1");
	...
	// define the position of the light and send the light position to your shader program
	_light_source2._lightPos = glm::vec4(75.0, 10.0, 50.0, 1.0);
	_light_source2._ambient_intensity = 0.0;
	_light_source2._specular_intensity = 0.1;
	_light_source2._diffuse_intensity = 0.5;
	_light_source2._attenuation_coeff = 0.05;
				 
	_light_source2._cone_angle = 15.0; // in degree
	_light_source2._cone_direction = glm::vec3(0.0, 0.0, -1.0); // this must be aligned with the object and light position.
				 
																// Read all the index values from the shader program
	_light_source2._ambientIdx = glGetUniformLocation(_program, "ambient_intensity_2");
	_light_source2._diffuseIdx = glGetUniformLocation(_program, "diffuse_intensity_2");
	_light_source2._specularIdx = glGetUniformLocation(_program, "specular_intensity_2");
	_light_source2._attenuation_coeffIdx = glGetUniformLocation(_program, "attenuationCoefficient_2");
	_light_source2._lightPosIdx = glGetUniformLocation(_program, "light_position_2");
	_light_source2._cone_angleIdx = glGetUniformLocation(_program, "cone_angle_2");
	_light_source2._cone_directionIdx = glGetUniformLocation(_program, "cone_direction_2");
