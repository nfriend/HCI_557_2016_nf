...
GLGreenSphere::GLGreenSphere(float center_x, float center_y, float center_z, float radius, int rows, int segments)
{
...
*/
void GLGreenSphere::initShader(void)
{
	// This loads the shader program from a file
	_program = LoadAndCreateShaderProgram("./spotlight_green.vs", "./spotlight_green.fs");
	...
	_light_source1._lightPos = glm::vec4(50.0, 10.0, 50.0, 1.0);
	_light_source1._ambient_intensity = 0.0;
	_light_source1._specular_intensity = 5.0;
	_light_source1._diffuse_intensity = 15.5;
	_light_source1._attenuation_coeff = 0.00;

	_light_source1._cone_angle = 15.0; // in degree
	_light_source1._cone_direction = glm::vec3(0.0, 0.0, -1.0); // this must be aligned with the object and light position.
